package io.paradaux.jobs.testsupport;
 ...
 *       needed.</li>
 *   <li><b>External (CI)</b>: when {@code JOBS_TEST_JDBC_URL} is set, point
...
public final class MariadbContainerExtension {
    ...
    private static final String DATABASE_NAME = "jobs_test";

    /** When set, the harness uses this URL instead of booting an embedded DB. */
    private static final String EXTERNAL_URL_ENV  = "JOBS_TEST_JDBC_URL";
    private static final String EXTERNAL_USER_ENV = "JOBS_TEST_DB_USER";
    private static final String EXTERNAL_PASS_ENV = "JOBS_TEST_DB_PASS";
     ...
     * triggers and the UUID helper functions the job mappers rely on.
     */
    ...
    private static HikariDataSource buildExternalDataSource(String jdbcUrl) {
        ...
        cfg.setPoolName("jobs-tests-external");
    ...
    public static void truncateAll(DataSource dataSource) throws Exception {
        try (Connection conn = dataSource.getConnection();
            ...
            // Only the tables :jobs owns. The schema is the whole shared economy
            // database (built from the real economy-flyway migrations), but a job
            // test has no business truncating another plugin's tables.
            st.execute("TRUNCATE TABLE job_event");
            st.execute("TRUNCATE TABLE job_membership");
