 ...
 * {@link TransactionServiceImpl} (ADT-138): affordability (FLOOR to whole items),
...
class TransactionMathTest {
    ...
    private TransactionServiceImpl serviceWithPrecision(int precision) {
        ...
        return new TransactionServiceImpl(null, null, null, null, null, null, null, null,
    ...
    void affordableItems_floorsToWholeItems() {
        assertThat(TransactionServiceImpl.getAmountOfAffordableItems(new BigDecimal("10.00"), new BigDecimal("3.00"))).isEqualTo(3); // 3.33 -> 3
        assertThat(TransactionServiceImpl.getAmountOfAffordableItems(new BigDecimal("9.00"), new BigDecimal("3.00"))).isEqualTo(3);  // exact
    }

    @Test
    void affordableItems_isZeroWhenCannotAffordEvenOne() {
        assertThat(TransactionServiceImpl.getAmountOfAffordableItems(new BigDecimal("2.99"), new BigDecimal("3.00"))).isEqualTo(0);
        assertThat(TransactionServiceImpl.getAmountOfAffordableItems(BigDecimal.ZERO, new BigDecimal("3.00"))).isEqualTo(0);
    }
    ...
    void affordableItems_handlesNonTerminatingPerItemPrice() {
        ...
        assertThat(TransactionServiceImpl.getAmountOfAffordableItems(new BigDecimal("10.00"), pricePerItem)).isEqualTo(3);
    }
    ...
    void scalePrice_roundsHalfUpToConfiguredScale() {
        TransactionServiceImpl service = serviceWithPrecision(2);
    ...
    void scalePrice_honoursAHigherConfiguredPrecision() {
        TransactionServiceImpl service = serviceWithPrecision(4);
    ...
    void roundedToZero_trueOnlyWhenPositivePriceScalesToZero() {
        assertThat(TransactionServiceImpl.roundedToZero(new BigDecimal("0.001"), new BigDecimal("0.00"))).isTrue();
    }

    @Test
    void roundedToZero_falseForAFreeShop() {
        // pricePerItem 0 (free shop) is not "rounded to zero" — it is genuinely free.
        assertThat(TransactionServiceImpl.roundedToZero(BigDecimal.ZERO, BigDecimal.ZERO)).isFalse();
    }

    @Test
    void roundedToZero_falseWhenScaledIsPositive() {
        assertThat(TransactionServiceImpl.roundedToZero(new BigDecimal("0.01"), new BigDecimal("0.01"))).isFalse();
    }
}
